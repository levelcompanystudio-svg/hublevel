        ...
        <table className="w-full min-w-[760px] border-collapse text-left">
          <thead className="border-b border-border bg-surface text-xs uppercase tracking-wide text-muted-foreground">
            <tr>
              ...
              <th className="px-4 py-3.5 font-semibold">Preco padrao</th>
              <th className="px-4 py-3.5 font-semibold">Ciclo de cobranca</th>
          ...
          <tbody className="divide-y divide-border">
            {services.map((service) => (
              <tr key={service.id} className="bg-card transition-colors hover:bg-card-elevated">
                ...
                <td className="px-4 py-3 text-sm text-muted-foreground">{service.category || '-'}</td>
                <td className="px-4 py-3 text-sm text-muted-foreground">{formatPrice(service.default_price)}</td>
                <td className="px-4 py-3">
